...
import com.mao.maocodemother.innerservice.InnerScreenshotService;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public Long createApp(AppAddRequest appAddRequest, User loginUser) {
        ...
        // 应用名称：优先使用用户指定名称，否则取 initPrompt 前 12 位
        if (StrUtil.isBlank(app.getAppName())) {
            app.setAppName(initPrompt.substring(0, Math.min(initPrompt.length(), 12)));
        }
        // 代码生成类型：优先使用用户指定类型，否则由 AI 智能选择（多例模式）
        CodeGenTypeEnum selectedCodeGenType;
        String reqCodeGenType = appAddRequest.getCodeGenType();
        if (StrUtil.isNotBlank(reqCodeGenType)) {
            CodeGenTypeEnum enumByValue = CodeGenTypeEnum.getEnumByValue(reqCodeGenType);
            ThrowUtils.throwIf(enumByValue == null, ErrorCode.PARAMS_ERROR, "代码生成类型不合法");
            selectedCodeGenType = enumByValue;
        } else {
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        // 10. 构建应用访问 URL（与静态资源服务的 /static/{deployKey} 路径保持一致，便于通过网关访问）
        String appDeployUrl = String.format("%s/static/%s/", deployHost, deployKey);        // 11. 异步生成截图并且更新应用封面
    ...
    @Override
    public List<AppVO> listRecentApps(Long userId, int limit) {
        if (userId == null) {
            return new ArrayList<>();
        }
        List<App> appList = this.list(QueryWrapper.create()
                .eq("userId", userId)
                .orderBy("COALESCE(lastOpenTime, createTime)", false)
                .limit(limit));
        return getAppVOList(appList);
    }

    @Override
    public boolean markAppOpened(Long appId, Long userId) {
        ThrowUtils.throwIf(appId == null || appId <= 0 || userId == null, ErrorCode.PARAMS_ERROR);
        App app = new App();
        app.setLastOpenTime(LocalDateTime.now());
        return this.update(app, QueryWrapper.create()
                .eq("id", appId)
                .eq("userId", userId));
    }
