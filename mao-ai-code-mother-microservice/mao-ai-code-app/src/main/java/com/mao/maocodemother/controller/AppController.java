...
public class AppController {
    ...
    @Resource
    private com.mao.maocodemother.mq.CodeGenerationProducer codeGenerationProducer;
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId,
        ...
        return contentFlux
                ...
                .onErrorResume(e -> {
                    // 向前端发送业务错误事件，便于前端展示具体错误信息
                    String errorMessage = e.getMessage() != null ? e.getMessage() : "生成失败，请稍后重试";
                    Map<String, String> errorWrapper = Map.of("message", errorMessage);
                    return Mono.just(
    ...
    public BaseResponse<Long> addApp(@RequestBody AppAddRequest appAddRequest, HttpServletRequest request) {
        ...
        // 发送异步代码生成任务到 RabbitMQ（由消费者处理后置逻辑）
        codeGenerationProducer.sendGenerationTask(
                new com.mao.maocodemother.mq.GenerationTaskMessage(
                        appId, loginUser.getId(), appAddRequest.getAppName(),
                        appAddRequest.getCodeGenType(), appAddRequest.getInitPrompt()));
    ...
    public BaseResponse<List<AppVO>> listRecentApps(HttpServletRequest request,
        ...
        User loginUser = InnerUserService.getLoginUser(request);
        List<AppVO> list = appService.listRecentApps(loginUser.getId(), limit);
        return ResultUtils.success(list);
    }

    /**
     * 记录应用被打开，更新最近打开时间（用于“最近项目”跨设备同步）
     *
     * @param appId  应用 id
     * @param request 请求
     * @return 是否更新成功
    ...
    public BaseResponse<Boolean> markAppOpened(@PathVariable Long appId, HttpServletRequest request) {
        User loginUser = InnerUserService.getLoginUser(request);
        boolean result = appService.markAppOpened(appId, loginUser.getId());
        return ResultUtils.success(result);
    }
