...
public class AiCodeGeneratorServiceFactory {
    ...
    private volatile RetrievalAugmentor retrievalAugmentor;
     ...
     */
    /**
     * 构建（并缓存）检索增强器：将 Milvus 向量检索结果注入生成提示词，实现 RAG。
     * 用 {@link SafeRetrievalAugmentor} 包裹，Milvus / Embedding 不可用时自动降级，不阻断生成。
     */
    private RetrievalAugmentor getRetrievalAugmentor() {
        if (this.retrievalAugmentor == null) {
            synchronized (this) {
                if (this.retrievalAugmentor == null) {
                    EmbeddingStoreContentRetriever retriever = vectorSearchService.toContentRetriever(ragTopK, ragMinScore);
                    this.retrievalAugmentor = new SafeRetrievalAugmentor(
        ...
        return switch (codeGenType) {
            ...
            case VUE_PROJECT -> {
                ...
                yield AiServices.builder(AiCodeGeneratorService.class)
                        ...
                        .retrievalAugmentor(getRetrievalAugmentor()) // 添加检索增强（RAG）
