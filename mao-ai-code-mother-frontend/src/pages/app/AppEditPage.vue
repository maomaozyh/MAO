      ...
      <a-card title="基本信息" :loading="loading">
        ...
        >
          ...
          <a-form-item label="可见范围" name="isPublic">
            <a-switch
              v-model:checked="isPublicOpen"
              checked-children="公开"
              un-checked-children="私密"
            />
            <div class="form-tip">私密应用仅你自己与管理员可见，不会出现在广场 / 发现</div>
          </a-form-item>
...
const isPublicOpen = computed({
  get: () => formData.isPublic === 1,
  set: (v: boolean) => {
    formData.isPublic = v ? 1 : 0
  },
})
...
const fetchAppInfo = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      formData.isPublic = appInfo.value.isPublic ?? 1
    } else {
