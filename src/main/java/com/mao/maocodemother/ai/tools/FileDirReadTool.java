...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(
        ...
        try {
            // 安全校验：防止路径遍历
            Path path = PathSecurityUtils.safeResolveDir(
                    relativeDirPath == null ? "" : relativeDirPath, appId, PROJECT_DIR_PREFIX);
            File targetDir = path.toFile();
            if (!targetDir.exists()) {
                return "错误：目录不存在 - " + relativeDirPath;
            }
            if (!targetDir.isDirectory()) {
                return "错误：指定路径不是目录 - " + relativeDirPath;
            }
            StringBuilder structure = new StringBuilder();
            structure.append("项目目录结构:\n");
            // 使用 Hutool 递归获取所有文件
            List<File> allFiles = FileUtil.loopFiles(targetDir, file -> !shouldIgnore(file.getName()));
            // 限制返回文件数量，避免 AI 上下文溢出
            if (allFiles.size() > 200) {
                return "错误：目录文件过多（" + allFiles.size() + " 个），超过 200 个限制，请指定子目录读取";
            }
            // 按路径深度和名称排序显示
            allFiles.stream()
                    .sorted((f1, f2) -> {
            ...
            log.warn("目录读取安全拦截：appId={}, path={}, reason={}", appId, relativeDirPath, e.getMessage());
